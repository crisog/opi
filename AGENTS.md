...
- Use the Node.js version pinned in `.nvmrc` and `.node-version` and the npm version pinned in `packageManager`. Keep root engine requirements aligned with them. Published package engines must reflect their actual runtime minimum instead of the development toolchain.
